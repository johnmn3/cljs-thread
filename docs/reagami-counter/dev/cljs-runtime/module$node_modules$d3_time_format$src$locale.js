shadow$provide.module$node_modules$d3_time_format$src$locale = function(global, require, module, exports) {
  ...
  function formatLocale(locale) {
      ...
      return function(date) {
        ...
        for (date instanceof Date || (date = new Date(+date)); ++i < n;) {
          if (37 === specifier.charCodeAt(i)) {
            ...
            null != (j = pads[c = specifier.charAt(++i)]) ? c = specifier.charAt(++i) : j = "e" === c ? " " : "0";
            if (format = formats[c]) {
              c = format(date, j);
            }
            string.push(c);
    ...
    function newParse(specifier, Z) {
      return function(string) {
        ...
        if ("s" in d) {
          return new Date(1000 * d.s + ("L" in d ? d.L : 0));
        }
        !Z || "Z" in d || (d.Z = 0);
        ...
        if ("V" in d) {
          ...
          if ("Z" in d) {
            ...
            string = 4 < day || 0 === day ? require$d3_time.utcMonday.ceil(string) : (0,require$d3_time.utcMonday)(string);
            string = require$d3_time.utcDay.offset(string, 7 * (d.V - 1));
            d.y = string.getUTCFullYear();
            d.m = string.getUTCMonth();
            d.d = string.getUTCDate() + (d.w + 6) % 7;
          } else {
            string = localDate(newDate(d.y, 0, 1)), day = string.getDay(), string = 4 < day || 0 === day ? require$d3_time.timeMonday.ceil(string) : (0,require$d3_time.timeMonday)(string), string = require$d3_time.timeDay.offset(string, 7 * (d.V - 1)), d.y = string.getFullYear(), d.m = string.getMonth(), d.d = string.getDate() + (d.w + 6) % 7;
          }
        } else if ("W" in d || "U" in d) {
          "w" in d || (d.w = "u" in d ? d.u % 7 : "W" in d ? 1 : 0), day = "Z" in d ? utcDate(newDate(d.y, 0, 1)).getUTCDay() : localDate(newDate(d.y, 0, 1)).getDay(), d.m = 0, d.d = "W" in d ? (d.w + 6) % 7 + 7 * d.W - (day + 5) % 7 : d.w + 7 * d.U - (day + 6) % 7;
        }
    ...
    function parseSpecifier(d, specifier, string, j) {
      for (var i = 0, n = specifier.length, m = string.length, c; i < n;) {
        ...
        if (37 === c) {
          if (c = specifier.charAt(i++), c = parses[c in pads ? specifier.charAt(i++) : c], !c || 0 > (j = c(d, string, j))) {
            return -1;
          }
  ...
  function parseYear(d, string, i) {
    return (string = numberRe.exec(string.slice(i, i + 2))) ? (d.y = +string[0] + (68 < +string[0] ? 1900 : 2000), i + string[0].length) : -1;
  }
  function parseZone(d, string, i) {
    return (string = /^(Z)|([+-]\d\d)(?::?(\d\d))?/.exec(string.slice(i, i + 6))) ? (d.Z = string[1] ? 0 : -(string[2] + (string[3] || "00")), i + string[0].length) : -1;
  }
  function parseQuarter(d, string, i) {
    return (string = numberRe.exec(string.slice(i, i + 1))) ? (d.q = 3 * string[0] - 3, i + string[0].length) : -1;
  }
  ...
  function dISO(d) {
    var day = d.getDay();
    return 4 <= day || 0 === day ? (0,require$d3_time.timeThursday)(d) : require$d3_time.timeThursday.ceil(d);
  }
  function formatWeekNumberISO(d, p) {
    d = dISO(d);
    return pad(require$d3_time.timeThursday.count((0,require$d3_time.timeYear)(d), d) + (4 === (0,require$d3_time.timeYear)(d).getDay()), p, 2);
  }
  ...
  function formatFullYearISO(d, p) {
    var day = d.getDay();
    d = 4 <= day || 0 === day ? (0,require$d3_time.timeThursday)(d) : require$d3_time.timeThursday.ceil(d);
    return pad(d.getFullYear() % 10000, p, 4);
  }
  function formatZone(d) {
    d = d.getTimezoneOffset();
    return (0 < d ? "-" : (d *= -1, "+")) + pad(d / 60 | 0, "0", 2) + pad(d % 60, "0", 2);
  }
  ...
  function UTCdISO(d) {
    var day = d.getUTCDay();
    return 4 <= day || 0 === day ? (0,require$d3_time.utcThursday)(d) : require$d3_time.utcThursday.ceil(d);
  }
  function formatUTCWeekNumberISO(d, p) {
    d = UTCdISO(d);
    return pad(require$d3_time.utcThursday.count((0,require$d3_time.utcYear)(d), d) + (4 === (0,require$d3_time.utcYear)(d).getUTCDay()), p, 2);
  }
  ...
  function formatUTCFullYearISO(d, p) {
    var day = d.getUTCDay();
    d = 4 <= day || 0 === day ? (0,require$d3_time.utcThursday)(d) : require$d3_time.utcThursday.ceil(d);
    return pad(d.getUTCFullYear() % 10000, p, 4);
  }
  ...
  var require$d3_time = require("module$node_modules$d3_time$src$index"), pads = {"-":"", _:" ", 0:"0"}, numberRe = /^\s*\d+/, percentRe = /^%/, requoteRe = /[\\^$*+?|[\]().{}]/g;
};

//# sourceMappingURL=module$node_modules$d3_time_format$src$locale.js.map
