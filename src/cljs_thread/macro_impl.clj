...
(defn parse-in [x]
  (if-not (coll? x)
    ...
    (let [f (first x)
          s (second x)]
      (cond
        ;; Qualified keyword as first arg -> {:id f} shorthand
        (and (keyword? f) (namespace f))
        [[] {:id f} (vec (rest x))]

        (and (vector? f) (map? s)) [f s (rest (rest x))]
        (vector? f) [f {} (rest x)]
        (map? f) [[] f (rest x)]
        :else [[] {} (vec x)]))))
...
(defn yield-form? [form]
  (when (seq? form)
    (seq (filter #(or (= % 'yield) (= % 'cljs-thread.core/yield)) form))))
