(ns cljs-thread.strategy.node-blob-bootstrap-test
  "Full integration test for Strategy 2 (Blob Bootstrap) on Node.js.
   Installs blob-bootstrap, then runs all 8 parallel tests."
...
(defn main []
  (println "=== Strategy 2: Blob Bootstrap (Node.js) ===")
  (println "Node.js:" js/process.version)

  (let [path (js* "require('path')")
        worker-path (.resolve path (.dirname path js/__filename) "worker.js")]

    ;; Install blob-bootstrap — workers created via eval + require
    (blob-bootstrap/install! {:scripts [worker-path]})
