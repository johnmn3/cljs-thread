(ns cljs-thread.strategy.node-live-kernel-test
  "Full integration test for Strategy 4 (Live Kernel) on Node.js.
   Installs live-kernel, then runs all 8 parallel tests."
...
(defn main []
  (println "=== Strategy 4: Live Kernel (Node.js) ===")
  (println "Node.js:" js/process.version)

  (let [path (js* "require('path')")
        worker-path (.resolve path (.dirname path js/__filename) "worker.js")]

    ;; Install live-kernel — workers boot as minimal kernels, then load runtime
    (live-kernel/install! {:scripts [worker-path]})
