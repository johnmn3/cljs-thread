(ns cljs-thread.strategy.node-eval-kernel-test
  "Full integration test for Strategy 3 (Eval Kernel) on Node.js.
   Installs eval-kernel, then runs all 8 parallel tests."
...
(defn main []
  (println "=== Strategy 3: Eval Kernel (Node.js) ===")
  (println "Node.js:" js/process.version)

  (let [path (js* "require('path')")
        worker-path (.resolve path (.dirname path js/__filename) "worker.js")]

    ;; Install eval-kernel — workers boot as minimal kernels, then load runtime
    (eval-kernel/install! {:scripts [worker-path]})
