(ns cljs-thread.strategy.node-fat-kernel-test
  "Full integration test for Strategy 5 (Fat Kernel) on Node.js.
   Installs fat-kernel, then runs all 8 parallel tests.

   The fat-kernel strategy reads the worker.js source once,
   caches it, and creates eval workers with the full runtime inlined.
   No require() calls from the worker side — everything is in the blob."
