(ns cljs-thread.runner.thread-test-main
  ...
  (:require
   ...
   [cljs-thread.eve.mem-test :as mem-test]
   [cljs-thread.eve.mem :as mem]
   [cljs-thread.eve.mmap-test :as mmap-test]
   [cljs-thread.eve.mmap-worker-test :as mmap-worker-test]
   [cljs-thread.eve.mmap-slab-test :as mmap-slab-test]
   [cljs-thread.eve.mmap-atom-test :as mmap-atom-test]
   [cljs-thread.eve.mmap-atom-e2e-test :as mmap-atom-e2e-test]))
...
(goog/exportSymbol "cljs_thread.eve.mem_test" mem-test)
(goog/exportSymbol "cljs_thread.eve.mmap_test" mmap-test)
(goog/exportSymbol "cljs_thread.eve.mmap_worker_test" mmap-worker-test)
(goog/exportSymbol "cljs_thread.eve.mmap_slab_test" mmap-slab-test)
(goog/exportSymbol "cljs_thread.eve.mmap_atom_test" mmap-atom-test)
(goog/exportSymbol "cljs_thread.eve.mmap_atom_e2e_test" mmap-atom-e2e-test)
...
(defn- run-mem! []
  (t/run-tests 'cljs-thread.eve.mem-test))

(defn- run-mmap! []
  (let [path (.resolve (js/require "path") "build/Release/mmap_cas.node")]
    (mem/load-native-addon! (js/require path)))
  (t/run-tests 'cljs-thread.eve.mmap-test
               'cljs-thread.eve.mmap-worker-test))

(defn- run-mmap-slab! []
  (let [path (.resolve (js/require "path") "build/Release/mmap_cas.node")]
    (mem/load-native-addon! (js/require path)))
  (eve-alloc/init-mmap-slab! 5 "/tmp/eve-p3-cls5.mem")
  (t/run-tests 'cljs-thread.eve.mmap-slab-test))

(defn- run-mmap-atom! []
  (let [path (.resolve (js/require "path") "build/Release/mmap_cas.node")]
    (mem/load-native-addon! (js/require path)))
  (t/run-tests 'cljs-thread.eve.mmap-atom-test))

(defn- run-mmap-atom-e2e! []
  (let [path (.resolve (js/require "path") "build/Release/mmap_cas.node")]
    (mem/load-native-addon! (js/require path)))
  ;; Note: NOT added to run-all! — mmap init conflicts with SAB-backed tests
  (t/run-tests 'cljs-thread.eve.mmap-atom-e2e-test))
