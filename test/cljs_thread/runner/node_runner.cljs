(ns cljs-thread.runner.node-runner
...
(defn test-in-nested []
  (-> @(in :core (+ 1 @(in :db (+ 2 3))))
      (.then (fn [result]
               (check "in-nested" 6 result)))
      (.catch (fn [e]
                (fail! "in-nested" (str "error: " e))))))
...
(defn main []
  ...
  ;; Need :core, :future, at least one :fp-* for future tests,
  ;; and at least one :injest-* for parallel transducer tests
  (on-when (and (contains? @s/peers :core)
                (contains? @s/peers :future)
                (some #(.startsWith (name %) "fp-") (keys @s/peers))
                (some #(.startsWith (name %) "injest-") (keys @s/peers)))
    {:max-time 30000}
